 ...
 * @note
 * 使用流程:
 * 1. 先由主函数统一初始化共用的软件IIC, 再调用OLED_Init()初始化OLED. 注意STM32启动比OLED上电快, 可等待20ms再初始化OLED
...
#define OLED_IIC_SCL_PIN GPIO_PIN_7
#define OLED_IIC_SDA_PIN GPIO_PIN_6

#define OLED_SCL_1() HAL_GPIO_WritePin(OLED_IIC_PORT, OLED_IIC_SCL_PIN, GPIO_PIN_SET)
#define OLED_SCL_0() HAL_GPIO_WritePin(OLED_IIC_PORT, OLED_IIC_SCL_PIN, GPIO_PIN_RESET)
#define OLED_SDA_1() HAL_GPIO_WritePin(OLED_IIC_PORT, OLED_IIC_SDA_PIN, GPIO_PIN_SET)
#define OLED_SDA_0() HAL_GPIO_WritePin(OLED_IIC_PORT, OLED_IIC_SDA_PIN, GPIO_PIN_RESET)
#define OLED_SDA_READ() HAL_GPIO_ReadPin(OLED_IIC_PORT, OLED_IIC_SDA_PIN)

// OLED常见器件地址(8bit地址)
#define OLED_ADDRESS_3C 0x78
#define OLED_ADDRESS_3D 0x7A
...
static uint8_t g_oled_address = OLED_ADDRESS_3C;
...
static void OLED_IIC_Init(void)
{
  GPIO_InitTypeDef GPIO_InitStructure;

  __HAL_RCC_GPIOA_CLK_ENABLE();

  GPIO_InitStructure.Pin = OLED_IIC_SCL_PIN | OLED_IIC_SDA_PIN;
  GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH;
  GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_OD;
  HAL_GPIO_Init(OLED_IIC_PORT, &GPIO_InitStructure);
...
static uint8_t OLED_IIC_WaitAck(void)
{
  uint8_t ack;
  ...
  OLED_SCL_1();
  OLED_IIC_Delay();
  ack = OLED_SDA_READ() ? 1U : 0U;
  OLED_SCL_0();
  OLED_IIC_Delay();
  return ack;
}

static uint8_t OLED_CheckAddress(uint8_t address)
{
  uint8_t ack;

  OLED_IIC_Start();
  OLED_IIC_SendByte(address);
  ack = OLED_IIC_WaitAck();
  OLED_IIC_Stop();

  return (ack == 0U);
}

uint8_t OLED_DetectAddress(void)
{
  if (OLED_CheckAddress(OLED_ADDRESS_3C))
  {
    g_oled_address = OLED_ADDRESS_3C;
    g_oled_ready = 1U;
    return 1U;
  }

  if (OLED_CheckAddress(OLED_ADDRESS_3D))
  {
    g_oled_address = OLED_ADDRESS_3D;
    g_oled_ready = 1U;
    return 1U;
  }
...
{
  OLED_IIC_Start();
  OLED_IIC_SendByte(g_oled_address);
  if (OLED_IIC_WaitAck())
  {
    g_oled_ready = 0U;
    OLED_IIC_Stop();
